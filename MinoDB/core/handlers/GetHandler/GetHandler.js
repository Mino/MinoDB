...
function GetHandler(api, user, parameters, callback) {
    ...
    validator.get("addresses", BasicVal.array(true), BasicVal.each(function(this_address, index){
    ...
    for (var i=0; i < gh.paths.length; i++) {
        gh.get_path(gh.paths[i]);
    }

    for (i=0; i < gh.ids.length; i++) {
        gh.get_id(gh.ids[i]);
    }

    for (i=0; i < gh.types.length; i++) {
        gh.get_type(gh.types[i]);
    }
}
